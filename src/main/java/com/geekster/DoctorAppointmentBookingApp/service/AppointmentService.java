...
public class AppointmentService {
    ...
    public String scheduleAppointment(AuthenticationInputDto authInfo, Appointment appointment) {
        ...
        if(pTokenService.authenticate(authInfo)) {
            ...
            if(doc!= null)
            {
                ...
                return "appointment booked at time :" + appointment.getAppScheduledTime() + " with Dr. " + doc.getDocName() ;
            }
            else
            {
    ...
    public String cancelAppointment(AuthenticationInputDto authInfo, Integer appointment)
    {
        if(pTokenService.authenticate((authInfo)))
        {
            String email=authInfo.getEmail();
            Patient patient=patientRepo.findFirstByPatientEmail(email);
            Appointment existingAppointment=appointmentRepo.findById(appointment).orElseThrow();

            if(existingAppointment.getPatient().equals((patient)))
            {
                 appointmentRepo.deleteById(appointment);
                 return "appointment with doc has been canccled";
            }
            else
            {
                return "Un Authorized cancel appointment";
            }
        }
        else
        {
